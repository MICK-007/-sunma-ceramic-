...
import { uploadCmsMedia, deleteCmsMedia, getCmsMediaUrl } from '../utils/storage';

/**
 * UPLOAD FLOW AUDIT & CANONICAL URL PERSISTENCE VERIFICATION TEST
 */
export async function runUploadFlowAuditTest() {
  ...
  try {
    ...
    const canonicalUrl = getCmsMediaUrl(canonicalStoragePath);
    ...
    await deleteCmsMedia(canonicalStoragePath);
