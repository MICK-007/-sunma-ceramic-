...
async function runStep6SecurityAudit() {
  ...
  try {
    ...
    assert((insertedLog.details as any).password === undefined, 'Audit log details exclude plaintext passwords');
    assert((insertedLog.details as any).token === undefined, 'Audit log details exclude session/access tokens');
    ...
    const auditDocPath = path.join(rootDir, 'SECURITY-AUDIT.md');
    assert(fs.existsSync(auditDocPath), 'SECURITY-AUDIT.md artifact exists in repository root');
