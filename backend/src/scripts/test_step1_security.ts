...
async function runSecurityAudit() {
  ...
  try {
    ...
    try {
      jwt.verify(backdoorAdmin, jwtSecret, { algorithms: ['HS256'] });
    } catch {
    ...
    try {
      jwt.verify(backdoorUser, jwtSecret, { algorithms: ['HS256'] });
    } catch {
    ...
    const validToken = jwt.sign(minimalPayload, jwtSecret, { expiresIn: '1h', algorithm: 'HS256' });

    const decoded = jwt.verify(validToken, jwtSecret, { algorithms: ['HS256'] }) as any;
    ...
    try {
      jwt.verify(tamperedToken, jwtSecret, { algorithms: ['HS256'] });
    } catch {
    ...
    // Expired Token Test
    const expiredToken = jwt.sign(minimalPayload, jwtSecret, { expiresIn: '-1s', algorithm: 'HS256' });
