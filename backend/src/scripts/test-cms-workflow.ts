...
import { publishAdminCmsPage, getAdminCmsPageVersions, rollbackAdminCmsPage } from '../controllers/cms.controller';
...
async function runCmsWorkflowTest() {
  ...
  const createMockReq = (params: any = {}, body: any = {}, user: any = { id: adminUserId, role: 'ADMIN' }) => ({
    params,
    body,
    user,
    ip: '127.0.0.1',
  ...
  await publishAdminCmsPage(reqPublish as any, resPublish as any);
  ...
  await getAdminCmsPageVersions(reqVersions as any, resVersions as any);
  ...
  if (versionsList.length > 1) {
    ...
    await rollbackAdminCmsPage(reqRollback as any, resRollback as any);
    ...
    await publishAdminCmsPage(reqPublish as any, resPublish2 as any);
