...
const createOrder = async (req, res) => {
    ...
    const payloadHash = crypto_1.default.createHash('sha256').update(JSON.stringify(canonicalPayload)).digest('hex');
          ...
          `;
            ...
            if (err.code === '23505') {
                if (err.constraint === 'orders_user_id_idempotency_key_key') {
                    ...
                    const sqlRetry = (0, db_1.getDbClient)();
                    if (sqlRetry) {
                        try {
    ...
    const subtotal = deduplicatedItems.reduce((sum, item) => {
        const prod = store_1.store.products.find(p => p.id === item.productId);
...
const getUserOrders = (req, res) => {
    ...
    const userId = req.user.id;
    const userOrders = store_1.store.orders.filter(o => o.userId === userId);
