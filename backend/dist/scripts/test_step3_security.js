...
async function runStep3SecurityAudit() {
    ...
    try {
        ...
        const userJwt = jsonwebtoken_1.default.sign(userTokenPayload, jwtSecret, { expiresIn: '1h', algorithm: 'HS256' });
        const decodedUser = jsonwebtoken_1.default.verify(userJwt, jwtSecret, { algorithms: ['HS256'] });
        ...
        assert(publicPromo.usage_count === undefined, 'Public promotion response excludes internal usage_count');
        assert(publicPromo.admin_notes === undefined, 'Public promotion response excludes internal admin_notes');
        // SUMMARY
