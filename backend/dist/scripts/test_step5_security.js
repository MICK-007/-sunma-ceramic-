...
async function runStep5SecurityAudit() {
    ...
    try {
      ...
      `;
        ...
        const hashA = crypto_1.default.createHash('sha256').update(JSON.stringify(canonicalPayloadA)).digest('hex');
        const hashB = crypto_1.default.createHash('sha256').update(JSON.stringify(canonicalPayloadB)).digest('hex');
